...
FILE=${BINTRAY_MODULE}_${BINTRAY_MODULE_VER}.zip
...
curl -s -T $FILE \
     ...
     https://api.bintray.com/content/$BINTRAY_OWNER/$BINTRAY_REPOS/$BINTRAY_MODULE/$BINTRAY_MODULE_VER/$FILE

curl -s -u $BINTRAY_OWNER:$BINTRAY_API_KEY \
   -X POST https://api.bintray.com/content/$BINTRAY_OWNER/$BINTRAY_REPOS/$BINTRAY_MODULE/$BINTRAY_MODULE_VER/publish